...
struct CivilTime {
  ...
  CivilTime() : year(2000), month(1), day(1), hour(12), minute(0), second(0), nanosecond(0) {}

  /**
   * @brief Construct from civil UTC components.
   */
  CivilTime(int32_t y, uint8_t mo, uint8_t d, uint8_t h = 0, uint8_t mi = 0, uint8_t s = 0,
            uint32_t ns = 0)
      : year(y), month(mo), day(d), hour(h), minute(mi), second(s), nanosecond(ns) {}

  /// Convert to the C FFI struct.
  tempoch_utc_t to_c() const { return {year, month, day, hour, minute, second, nanosecond}; }

  /// Create from the C FFI struct.
  static CivilTime from_c(const tempoch_utc_t &c) {
    return CivilTime(c.year, c.month, c.day, c.hour, c.minute, c.second, c.nanosecond);
  }
};
...
inline std::ostream &operator<<(std::ostream &os, const CivilTime &u) {
  ...
  os << u.year << '-' << std::setfill('0') << std::setw(2) << static_cast<int>(u.month) << '-'
     << std::setw(2) << static_cast<int>(u.day) << ' ' << std::setw(2) << static_cast<int>(u.hour)
     << ':' << std::setw(2) << static_cast<int>(u.minute) << ':' << std::setw(2)
